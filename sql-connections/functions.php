...
function table_name()
{
	...
	$result=mysqli_query($con,$con,$sql) or die("error in function table_name()".mysqli_error($con));
...
function column_name($idee)
{
	...
	$result=mysqli_query($con,$con,$sql) or die("error in function column_name()".mysqli_error($con));
	$row = mysqli_fetch_array($result);
	if(!$row)
...
function next_tryy()
{
	...
	mysqli_query($con,$con,$sql) or die("error in function next_tryy()". mysqli_error($con));
}
...
function view_attempts()
{
	...
	$result=mysqli_query($con,$con,$sql) ;
	$row = mysqli_fetch_array($result);
	if(!$row)
